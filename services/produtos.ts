...
export async function listarProdutos(empresaId: string) {
  ...
  return data.map((prod: any) => ({
    ...
    precoVenda: prod.preco_venda || 0,
    precoPromocional: prod.preco_promocional,
    custoTotal: prod.custo_total || 0,
    margemLucro: prod.margem_lucro || 0,
    ativo: prod.ativo,
...
export async function obterProduto(id: string) {
  ...
  return {
    ...
    margemLucro: data.margem_lucro || 0,
    ativo: data.ativo,
...
export async function listarInsumosProduto(produtoId: string) {
  ...
  return data.map((item: any) => ({
    ...
    unidadeMedida: item.unidade_medida || 'g',
    custo: item.custo || 0,
    insumo: item.insumo ? {
      id: item.insumo.id,
...
export async function atualizarCustoTotalProduto(produtoId: string) {
  try {
